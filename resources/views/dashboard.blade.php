...
<head>
    ...
    <title>Pemantauan Suhu Luar Ruangan</title>
...
<body class="bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <nav class="flex space-x-8">
                    <a href="#"
                        class="text-blue-600 border-b-2 border-blue-600 pb-2 text-sm font-medium">Dashboard</a>
                    <a href="#" class="text-gray-600 hover:text-gray-900 text-sm font-medium">Lokasi</a>
                    <a href="#" class="text-gray-600 hover:text-gray-900 text-sm font-medium">Laporan</a>
                    <a href="#" class="text-gray-600 hover:text-gray-900 text-sm font-medium">Kesehatan</a>
                </nav>
            </div>
        </div>
    </header>
            ...
            <div class="lg:col-span-2">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div class="text-center">
                        <span id="average-temp"
                            class="text-6xl font-bold text-green-500">{{ number_format($averageTemp, 1) }}</span>
                        <span class="text-2xl text-gray-600">°C</span>
                    </div>
                    <p class="text-center text-gray-600 mt-2">Rata-rata di semua lokasi yang dipantau, berdasarkan data
                        sensor yang disimpan.</p>
                    ...
                    <div id="warning-message" class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-6">
                        <div class="flex items-center">
                            ...
                            <span id="warning-text" class="text-sm text-yellow-800">Saat ini Suhu Area Kampus kurang
                                sehat. Diharapkan Mahasiswa dan Pegawai minum air yang banyak.</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <!-- Kantin Pertanian -->
                        <div id="card-kantin"
                            class="bg-white rounded-lg shadow-sm p-4 {{ $data['kantin']['border'] }} border-l-4">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-gray-800">Kantin Pertanian</h4>
                                <i id="icon-kantin"
                                    class="{{ $data['kantin']['icon'] }} {{ $data['kantin']['text'] }}"></i>
                            </div>
                            <div id="temp-kantin" class="text-3xl font-bold {{ $data['kantin']['text'] }} mb-1">
                                {{ number_format($data['kantin']['nilai_temperatur'], 1) }}<span
                                    class="text-sm">°C</span></div>
                            <div id="status-kantin" class="text-sm text-gray-600 mb-3">Status:
                                {{ $data['kantin']['status'] }}</div>
                            <div class="h-12 mb-3">
                                <canvas id="chart-kantin" width="100" height="30"></canvas>
                            </div>
                            <button class="w-full bg-gray-100 text-gray-700 py-2 rounded text-sm hover:bg-gray-200">
                                <i class="fas fa-chart-line mr-1"></i> Detail
                            </button>
                        </div>

                        <!-- Lapangan Futsal -->
                        <div id="card-lapangan"
                            class="bg-white rounded-lg shadow-sm p-4 {{ $data['lapangan']['border'] }} border-l-4">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-gray-800">Lapangan Futsal</h4>
                                <i id="icon-lapangan"
                                    class="{{ $data['lapangan']['icon'] }} {{ $data['lapangan']['text'] }}"></i>
                            </div>
                            <div id="temp-lapangan" class="text-3xl font-bold {{ $data['lapangan']['text'] }} mb-1">
                                {{ number_format($data['lapangan']['nilai_temperatur'], 1) }}<span
                                    class="text-sm">°C</span></div>
                            <div id="status-lapangan" class="text-sm text-gray-600 mb-3">Status:
                                {{ $data['lapangan']['status'] }}</div>
                            <div class="h-12 mb-3">
                        ...
                        <!-- Parkiran -->
                        <div id="card-parkiran"
                            class="bg-white rounded-lg shadow-sm p-4 {{ $data['parkiran']['border'] }} border-l-4">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-gray-800">Parkiran</h4>
                                <i id="icon-parkiran"
                                    class="{{ $data['parkiran']['icon'] }} {{ $data['parkiran']['text'] }}"></i>
                            </div>
                            <div id="temp-parkiran" class="text-3xl font-bold {{ $data['parkiran']['text'] }} mb-1">
                                {{ number_format($data['parkiran']['nilai_temperatur'], 1) }}<span
                                    class="text-sm">°C</span></div>
                            <div id="status-parkiran" class="text-sm text-gray-600 mb-3">Status:
                                {{ $data['parkiran']['status'] }}</div>
                            <div class="h-12 mb-3">
                        ...
                        <!-- Foodcourt -->
                        <div id="card-food_court"
                            class="bg-white rounded-lg shadow-sm p-4 {{ $data['food_court']['border'] }} border-l-4">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-gray-800">Foodcourt</h4>
                                <i id="icon-food_court"
                                    class="{{ $data['food_court']['icon'] }} {{ $data['food_court']['text'] }}"></i>
                            </div>
                            <div id="temp-food_court"
                                class="text-3xl font-bold {{ $data['food_court']['text'] }} mb-1">
                                {{ number_format($data['food_court']['nilai_temperatur'], 1) }}<span
                                    class="text-sm">°C</span></div>
                            <div id="status-food_court" class="text-sm text-gray-600 mb-3">Status:
                                {{ $data['food_court']['status'] }}</div>
                            <div class="h-12 mb-3">
                        ...
                        <!-- Taman -->
                        <div id="card-taman"
                            class="bg-white rounded-lg shadow-sm p-4 {{ $data['taman']['border'] }} border-l-4">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-gray-800">Taman</h4>
                                <i id="icon-taman"
                                    class="{{ $data['taman']['icon'] }} {{ $data['taman']['text'] }}"></i>
                            </div>
                            <div id="temp-taman" class="text-3xl font-bold {{ $data['taman']['text'] }} mb-1">
                                {{ number_format($data['taman']['nilai_temperatur'], 1) }}<span
                                    class="text-sm">°C</span></div>
                            <div id="status-taman" class="text-sm text-gray-600 mb-3">Status:
                                {{ $data['taman']['status'] }}</div>
                            <div class="h-12 mb-3">
                        ...
                        <!-- Suhu Real-Time -->
                        <div id="card-real_time"
                            class="bg-white rounded-lg shadow-sm p-4 {{ $data['real_time']['border'] }} border-l-4">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-gray-800">Suhu Real-Time</h4>
                                <i id="icon-real_time"
                                    class="{{ $data['real_time']['icon'] }} {{ $data['real_time']['text'] }}"></i>
                            </div>
                            <div id="temp-real_time"
                                class="text-3xl font-bold {{ $data['real_time']['text'] }} mb-1">
                                {{ number_format($data['real_time']['nilai_temperatur'], 1) }}<span
                                    class="text-sm">°C</span></div>
                            <div id="status-real_time" class="text-sm text-gray-600 mb-3">Status:
                                {{ $data['real_time']['status'] }}</div>
                            <div class="h-12 mb-3">
            ...
            <div class="lg:col-span-1">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <div id="highest-temp" class="text-2xl font-bold text-red-500">
                            {{ number_format($highestTemp, 1) }}°C</div>
                    ...
                    <div class="mb-4">
                        ...
                        <div id="lowest-temp" class="text-2xl font-bold text-blue-500">
                            {{ number_format($lowestTemp, 1) }}°C</div>
                    ...
                    <div id="reports-container" class="space-y-3">
                        @foreach ($reports as $report)
                            <div class="{{ $report['bg'] }} border {{ $report['border'] }} rounded-lg p-3">
                                <div class="flex items-center">
                                    <i
                                        class="{{ $report['icon'] }} text-{{ $report['color'] }}-600 mr-2 text-sm"></i>
                                    <div>
                                        <div class="text-sm font-medium {{ $report['text'] }}">
                                            {{ $report['message'] }}</div>
                                        <div class="text-xs text-{{ $report['color'] }}-600">{{ $report['time'] }}
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        const updateDashboard = (data) => {
            ...
            if (data.averageTemp >= 35) {
                ...
                warningText.textContent =
                    'PERINGATAN CUACA EKSTREM! Suhu sangat tinggi. Hindari aktivitas luar ruangan dan pastikan minum air yang cukup.';
                ...
                warningText.textContent =
                    'Saat ini Suhu Area Kampus cukup tinggi. Diharapkan Mahasiswa dan Pegawai minum air yang banyak.';
            } else {
        ...
        const addUpdateIndicator = () => {
            ...
            indicator.className =
                'fixed top-4 right-4 bg-green-500 text-white px-3 py-1 rounded-full text-sm opacity-0 transition-opacity duration-300';
